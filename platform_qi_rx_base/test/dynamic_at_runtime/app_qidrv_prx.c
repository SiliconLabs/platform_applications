...
// can likely be smaller for most use cases
#define QI_TX_BUFFER_SIZE                     4096
// interval between QI code error messages
#define QI_CE_INTERVAL                        250
  ...
  while (1) {
    ...
    switch (cur_qi_step) {
      case 0:
        ...
        qi_status = qi_create_spi_stream_buffer(tx_buffer,
                                                &tx_index,
      ...
      default:
        ...
        if (qi_step == 1) {
          ...
          qi_status = qi_create_spi_stream_buffer(temp_buffer,
                                                  &temp_index,
                                                  &qi_predelay_control_error_message);
          EFM_ASSERT(qi_status == QI_OK);
          qi_generate_c_code("QI_control_error_PREDELAY",
                             emp_buffer,
                             temp_index);
        }
#endif
    ...
    ecode = SPIDRV_MTransmit(SPI_HANDLE,
                             tx_buffer,
                             tx_index,
                             transfer_callback);
    EFM_ASSERT(ecode == ECODE_OK);
