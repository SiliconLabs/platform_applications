...
#define NEGATIVE_CASE      0        // Used when a negative temperature is read
#define THREE_DIGIT_CASE   100000   // Used when a three-digit temperature is read

uint32_t rh_data = 0;               // Relative humidity data
int32_t temp_data = 0;              // Temperature data

static sl_sleeptimer_timer_handle_t periodic_timer; // Handle for periodic timer
 ...
 ******************************************************************************/
                                 ...
                                 void *data)
 {
   ...
   // Measure and read the values for Relative Humidity and Temperature
   sl_si70xx_measure_rh_and_temp(sl_i2cspm_sensor, SI7021_ADDR, &rh_data, &temp_data);
   sl_si70xx_read_rh_and_temp(sl_i2cspm_sensor, SI7021_ADDR, &rh_data, &temp_data);
