...
/***************************************************************************//**
 * @brief  Initializes the LCD to enable 1 COM x 8 SEG, set the frame rate to
...
void initLCD(void)
{
  ...
  CMU_ClockSelectSet(cmuClock_LCDCLK, cmuSelect_LFXO);
  ...
  LCD_Init_TypeDef lcd_init = {
    ...
    // Table 26.1 in the reference manual shows 0.0% charge redistribution with
  ...
  // Sequential write to AREGA to trigger "update event"
  LCD->AREGA = 0x01;
  // delay per LCD_E301 erratum
  sl_udelay_wait(2000);
  LCD->AREGA = anim_init.AReg;
