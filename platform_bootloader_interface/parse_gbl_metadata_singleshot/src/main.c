...
static const uint32_t encrypted_ring_wav_address =0x44000;

// size of OTA slot
static const uint32_t encrypted_ring_wav_size = 196608;
...
void metadata_parsing_function(void)
{
  ...
  retval = bootloader_parseBuffer((BootloaderParserContext_t *)ctx,
                              &callbacks, (uint8_t*)encrypted_ring_wav_address,
                              encrypted_ring_wav_size );
...
int main(void)
{
  ...
  printf("When ready press PushButton1 for GBL Signature verification and Parsing\n\r");
