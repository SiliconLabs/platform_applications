...
static volatile int     rxReadIndex = 0;        /**< Index in buffer to be read
                                                 */
static volatile int     rxWriteIndex = 0;       /**< Index in buffer to be
                                                 *   written to */
static volatile int     rxCount = 0;            /**< Keeps track of how much
                                                 *   data which are stored in
                                                 *   the buffer */
static volatile uint8_t rxBuffer[RXBUFSIZE];    /**< Buffer to store data */
static uint8_t          LFtoCRLF = 0;           /**< LF to CRLF conversion
                                                 *   disabled */
static bool             initialized = false;    /**< Initialize UART/LEUART */
#if defined(SL_CATALOG_POWER_MANAGER_PRESENT)
static bool             em1HasBeenRequired = false; /**< EM1 requirement
                                                     *   indicator */
#endif
...
void RETARGET_SerialInit(void)
{
  ...
  sl_gpio_set_pin_mode(RETARGET_TXPORT,
                       RETARGET_TXPIN,
                       SL_GPIO_MODE_PUSH_PULL,
                       1);
  sl_gpio_set_pin_mode(RETARGET_RXPORT,
                       RETARGET_RXPIN,
  ...
  init.clock_div = sl_hal_eusart_uart_calculate_clock_div(ref_freq,
                                                          115200,
                                                          init.oversampling);
  ...
  GPIO->EUSARTROUTE[RETARGET_UART_INDEX].TXROUTE = (RETARGET_TXPORT <<
                                                    _GPIO_EUSART_TXROUTE_PORT_SHIFT)
                                                   | (RETARGET_TXPIN <<
                                                      _GPIO_EUSART_TXROUTE_PIN_SHIFT);
  GPIO->EUSARTROUTE[RETARGET_UART_INDEX].RXROUTE = (RETARGET_RXPORT <<
  ...
  GPIO->USARTROUTE[RETARGET_UART_INDEX].TXROUTE = (RETARGET_TXPORT <<
                                                   _GPIO_USART_TXROUTE_PORT_SHIFT)
                                                  | (RETARGET_TXPIN <<
                                                     _GPIO_USART_TXROUTE_PIN_SHIFT);
  GPIO->USARTROUTE[RETARGET_UART_INDEX].RXROUTE = (RETARGET_RXPORT <<
