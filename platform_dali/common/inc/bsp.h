...
#define BSP_STATUS_OK                 0     /**< BSP API return code, no errors.
                                             */
#define BSP_STATUS_ILLEGAL_PARAM      (-1)  /**< BSP API return code, illegal
                                             *   input parameter. */
#define BSP_STATUS_NOT_IMPLEMENTED    (-2)  /**< BSP API return code, function
                                             *   not implemented (dummy). */
#define BSP_STATUS_UNSUPPORTED_MODE   (-3)  /**< BSP API return code,
                                             *   unsupported BSP mode. */
#define BSP_STATUS_IOEXP_FAILURE      (-4)  /**< BSP API return code,
                                             *   io-expander communication
                                             *   failed */

/* Initialization flag bitmasks for BSP_Init(). */
#define BSP_INIT_DK_SPI               0x01 /**< Mode flag for @ref BSP_Init(),
                                            *   init DK in SPI mode (DK3x50
                                            *   only). */
#define BSP_INIT_DK_EBI               0x02 /**< Mode flag for @ref BSP_Init(),
                                            *   init DK in EBI mode (DK3x50
                                            *   only). */
#define BSP_INIT_BCC                  0x04 /**< Mode flag for @ref BSP_Init(),
                                            *   init board controller
                                            *   communication. */
#define BSP_INIT_IOEXP                0x08 /**< Mode flag for @ref BSP_Init(),
                                            *   init io-expander on some radio
                                            *   boards */
...
/** Display Control */
typedef enum {
  BSP_Display_EBI,          /**< SSD2119 TFT controller driven by EFM32 EBI
                             *   interface */
  BSP_Display_SPI,          /**< SSD2119 TFT controller driven by EFM32 SPI
                             *   interface */
  BSP_Display_BC,           /**< SSD2119 TFT controller driven by board
                             *   controller (AEM) */
  ...
  BSP_Display_Mode8080,     /**< Configure SSD2119 for 8080 mode of operation
                             *    */
  BSP_Display_ModeGeneric,  /**< Configure SSD2119 for Generic+SPI mode of
                             *   operation */
} BSP_Display_TypeDef;
...
typedef enum {
  ...
  BSP_ACCEL_GSEL     = BC_PERCTRL_ACCEL_GSEL,     /**< Accelerometer range
                                                   *   select */
  BSP_ACCEL_SELFTEST = BC_PERCTRL_ACCEL_SELFTEST, /**< Accelerometer selftest
                                                   *   mode */
