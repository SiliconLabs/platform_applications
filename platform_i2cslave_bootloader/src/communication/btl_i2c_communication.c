...
extern bool bootloader_verify_application(void);
 ...
 */
bool handle_interrupt_level_commands(queue_t *rx_queue, queue_t *tx_queue)
{
  bool result = false;
    ...
    if (imageProps.contents & BTL_IMAGE_CONTENT_SE) {
      ...
      if (imageProps.contents & BTL_IMAGE_CONTENT_BOOTLOADER) {
        if (imageProps.bootloaderVersion > mainBootloaderTable->header.version) {
          ...
          bootload_commitBootloaderUpgrade(parser_getBootloaderUpgradeAddress(),
...
void verify(void)
{
  ...
  i2c_comm_current_operation = BOOT_OPERATION_VERIFY;
  bool result = bootloader_verify_application();
  // update status
  i2c_comm_current_operation = BOOT_OPERATION_NONE;
  if (result != 0) {
...
static void start_download(void)
{
  ...
  got_message = false;
  i2c_download_frame_data_t *downloadFrame = (i2c_download_frame_data_t*)message;
