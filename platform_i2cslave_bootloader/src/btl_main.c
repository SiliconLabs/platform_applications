...
#ifdef BTL_PLUGIN_I2C_ACTIVATION
#include "activation/i2c_activation.h"
#endif
...
const ApplicationProperties_t sl_app_properties = {
  ...
  .signatureLocation = ((uint32_t)&__rom_end__) - BTL_MAIN_STAGE_BASE
                       + ROM_END_SIZE,
  .app = {
...
#if defined(BOOTLOADER_SUPPORT_CERTIFICATES)
  // If certificate based boot chain is enabled, the bootloader binary will be
  // provided with a certificate that does not contain any key.
  // A valid certificate needs to be injected to the bootloader images using
...
void SystemInit2(void)
{
  ...
  // checking.
    ...
    && defined(APPLICATION_VERIFICATION_SKIP_EM4_RST)
...
#if defined(BOOTLOADER_ENFORCE_SECURE_BOOT) \
    && defined(APPLICATION_WRITE_DISABLE)
    // The neccessary check of valid signature pointer for application at
    // startOfAppSpace is already done in bootload_verifyApplication.
...
// *INDENT-ON*
#if defined(BTL_PLUGIN_I2C_ACTIVATION) \
    && defined(BOOTLOADER_SUPPORT_COMMUNICATION)
  i2c_activation_init();
#endif

#ifdef BTL_PLUGIN_GPIO_ACTIVATION
  result |= gpio_enterBootloader();
#endif

#ifdef BTL_PLUGIN_EZSP_GPIO_ACTIVATION
  result |= ezsp_gpio_enterBootloader();
#endif

#if defined(BTL_PLUGIN_I2C_ACTIVATION) \
    && defined(BOOTLOADER_SUPPORT_COMMUNICATION)
  if (!result)
    result |= i2c_enter_bootloader();
  i2c_activation_done();
#endif

  if (result)
    BTL_DEBUG_PRINT("entering to bootloader");
  return result;
}
...
bool bootloader_verify_application() {
  bool result = true;
  uint32_t startOfAppSpace = (uint32_t)mainStageTable.startOfAppSpace;
  ...
  // App should be verified
  if (result) {
    result = bootload_verifyApplication(startOfAppSpace);
  }
  return result;
}
