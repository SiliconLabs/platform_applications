...
void app_process_action(void)
{
   ...
   * the Watchdog timer will reset the system and bring it to a safe known state.
   *
   * For testing purposes, either of the push buttons can be used to stop the RTCC from updating the LCD,
  ...
  if(flag_time == 1) {
      ...
      update_lcd_date(g_time_info.year, g_time_info.month, g_time_info.day_of_month);
      update_lcd_time(g_time_info.hour, g_time_info.min, g_time_info.sec);

      /* If this condition is satisfied, the LCD is being updated regularly enough.
       * It would be better to keep the condition, count == 8 .
       * The issue is that WDOG might overflow while the count variable is being set to 8 and
       * WDOG needs a few perclk cycles to clear the counter. Thus, it would be better if the WDOG
       * is cleared a little earlier than intended.
       */

      flag_gpio = get_flag_gpio();
      if((count >=  6) && (flag_gpio == 0)) {
          feed_wdog();
        }

      /* Arbitrary reason to clear the LCD at this point.
       * It would be easier this way than if the LCD was cleared immediately after feeding WDOG
       */
      if(count == 8) {
          clear_lcd();
        }
