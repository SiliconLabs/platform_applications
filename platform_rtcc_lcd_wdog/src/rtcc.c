...
void init_rtcc(void)
{
  ...
  //Configure HFCLK because bus interface to Low Energy E peripherals is clocked by HFCLK(LE)
...
void get_date(struct time_s *time_data)
{
  ...
  time_data->day_of_month = (date & _RTCC_DATE_DAYOMU_MASK) +
                          (((date & _RTCC_DATE_DAYOMT_MASK) >> _RTCC_DATE_DAYOMT_SHIFT)* 10);

  time_data->month = ((date & _RTCC_DATE_MONTHU_MASK)>>_RTCC_DATE_MONTHU_SHIFT) +
                   (((date &_RTCC_DATE_MONTHT_MASK) >> _RTCC_DATE_MONTHT_SHIFT)* 10);

  time_data->year = ((date & _RTCC_DATE_YEARU_MASK)>> _RTCC_DATE_YEARU_SHIFT) +
                   (((date & _RTCC_DATE_YEART_MASK) >> _RTCC_DATE_YEART_SHIFT)* 10);
...
void get_time(struct time_s *time_data)
{
  ...
  time_data->sec = (time & _RTCC_TIME_SECU_MASK) +
                  (((time & _RTCC_TIME_SECT_MASK) >> _RTCC_TIME_SECT_SHIFT)* 10);

  time_data->min = ((time & _RTCC_TIME_MINU_MASK) >> _RTCC_TIME_MINU_SHIFT ) +
                  (((time & _RTCC_TIME_MINT_MASK) >> _RTCC_TIME_MINT_SHIFT)* 10);

  time_data->hour = ((time & _RTCC_TIME_HOURU_MASK)>> _RTCC_TIME_HOURU_SHIFT ) +
                  (((time & _RTCC_TIME_HOURT_MASK) >> _RTCC_TIME_HOURT_SHIFT)* 10);
}
